...
export function calculateDetailedScores(
  answers: (string | null)[],
): Record<University, UniversityDetailedScore> {
  ...
  questions.questions.forEach((q, index) => {
    ...
    unis.forEach((uni) => {
      ...
      const catResult = uniResult.categories.find(
        (c) => (c.category as string) === (categoryName as string),
      );
  ...
  unis.forEach((uni) => {
    ...
    uniResult.normalizedPercentage =
      totalRange > 0
        ? Math.round(
            ((uniResult.totalScore - uniResult.totalMinScore) / totalRange) *
              100,
          )
        : 0;
